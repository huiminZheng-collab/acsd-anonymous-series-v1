...
because it exists: every required key signs one `acsd-approval-target/v1`
...
- 51 Python tests passing offline, with one real-network test skipped;
...
- a Lean 4.33.1 build with 16 new PEC/composition theorems and no
...
- `paper/acsd-v2.tex`: content-anonymous manuscript source;
- `release-v2.1.1/`: deterministic candidate release tree. The earlier
  `release-v2.0.0/` and `release-v2.1.0/` trees are retained rather than
  overwritten.
...
python verify_release.py release-v2.1.1
```
...
Code and documentation in this v2 work tree are released under the MIT license;
