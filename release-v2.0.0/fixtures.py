...
(validate_pec, verify_disclosure, verify_dialogue_window, or
...
RFC3161_CAPABILITY = "rfc3161-exact-pec-imprint"
...
def corpus():
    ...
    cases.append(("social-claim-injected", lambda: _run(_social), "SOCIAL_CLAIM_FORBIDDEN"))
    ...
    def _foreign_git():
        ...
        verify_disclosure(d, pec, git_event)
    ...
    def _single_author():
        ...
        verify_disclosure(d, pec, git_event, required_approval_key_ids=AUTHOR_KEYS)
    ...
    def _git_time():
        verify_sidecar_subject({"pec_digest": digest(pec), "capability": "git-timestamp"}, pec, "EXTERNALLY_NOT_AFTER")
    cases.append(("git-time-upgrade", lambda: _run(_git_time), "TIME_CAPABILITY_MISSING"))

    def _witness_time():
        verify_sidecar_subject({"pec_digest": digest(pec), "capability": "witness-observation"}, pec, "EXTERNALLY_NOT_AFTER")
    cases.append(("witness-time-upgrade", lambda: _run(_witness_time), "TIME_CAPABILITY_MISSING"))

    def _valid_sidecar():
        verify_sidecar_subject({"pec_digest": digest(pec), "capability": RFC3161_CAPABILITY}, pec, "EXTERNALLY_NOT_AFTER")
    cases.append(("valid-rfc3161-sidecar", lambda: _run(_valid_sidecar), "VALID"))

    def _replayed():
        verify_sidecar_subject({"pec_digest": "0" * 64, "capability": RFC3161_CAPABILITY}, pec, "EXTERNALLY_NOT_AFTER")
