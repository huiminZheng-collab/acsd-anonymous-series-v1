...
fixture likewise uses placeholder digests and a synthetic work identifier to
exercise the binding rules; it does not demonstrate a real content binding.
Real releases are produced by the v1 fixture generator.
...
The released prototype uses Python for deterministic fixture generation and a zero-dependency Node.js verifier for strict CBOR/COSE and profile validation. The two paths are intentionally separate. The package is designed for offline replay: locked dependencies are included, and the verification driver regenerates fixtures, verifies a standalone release, independently verifies the series profile, builds a manifest, and verifies that manifest.

A command-line tool (\texttt{acsd.py}) now turns the profile into an executable workflow: \texttt{keygen} (Ed25519), \texttt{init} (a release directory from a manuscript and a team file), \texttt{approve} (per-author COSE Sign1 endorsement), \texttt{finalize} (signature-complete package assembly with optional RFC~3161 timestamping), and \texttt{verify} (offline re-verification with explicit outcomes and non-claims). The canonical JSON layer is exercised by a 64-vector Python--Node differential harness that enforces byte agreement across implementations; two divergences it surfaced (unpaired surrogate code points and tuples) are now rejected by both implementations.

\begin{table}[t]
\centering
\caption{Current executable evaluation corpus.}
\begin{tabular}{lr}
\toprule
...
Public-manifest entries & 111 / 111 verified \\
New PEC theorems in the abstract core & 4, no axiom dependency \\
\bottomrule
\end{tabular}
\end{table}
...
The new Lean development models four selected PEC acceptance contracts. It
proves that acceptance implies approval by every required pseudonymous key and
preservation of the no-social-claim policy, and that a missing required
approval or invalid event sequence precludes acceptance. The earlier v1 core
...
\bibitem{trusty}
T. Kuhn and M. Dumontier. \emph{Trusty URIs: Verifiable, Immutable, and Permanent Digital Artifacts for Linked Data}. ESWC, 2015. \url{https://arxiv.org/abs/1401.5775}
