...
| Delegate both public facades to one dependency-light bundle validator | Keeps compatibility while making PEC/governance/event/policy acceptance single-source | 13 shared adverse mutations compare exact first errors; all 148 Python tests pass | None in the current wire profile | low | attempted |
...
| Derive a claim only from a verified, exact-subject, compatible evidence atom | Makes every grant traceable to one typed support item | `claim_derivation.py`, 16/16 matrix decisions, three unsupported social claims denied | Production verifier certificate emission | low | attempted |
...
| Reuse one declarative compatibility relation and prove exact-to-abstract refinement for any subject projection | Makes the exact model a conservative strengthening of the coarse skeleton and connects strict text decoding to both levels | 81-theorem Lean build, 49-theorem axiom audit, formal-architecture regression | Production adapter facts remain an explicit assumption | low | attempted |
