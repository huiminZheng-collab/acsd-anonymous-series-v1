...
| Require predecessor quorum over every exact parent-to-child authority change | Prevent fresh-key lineage capture while allowing deliberate team/key rotation | `test_lineage_authorization.py`; COSE payload binds exact transition; the zero-dependency Node verifier independently checks a Python-produced transition authorization | Independent Node recomputation of the structural transition body | medium | attempted |
| Reuse child approvals when the authority key set is unchanged | Avoid duplicate signatures while still binding exact child bytes | Child target contains transition digest and is signed by every child/old key | Formal/executable refinement | low | attempted |
...
| Bind `(approval_set_digest, not_after_utc)` through receipt verification, transcript, and Lean appraisal | Preserves the exact RFC 3161 assertion across every decision layer | Checked-in freeTSA fixture; v3 Python/Node certificate agreement; Python/Lean adverse differential cases | Authorized-lineage transcript and second-domain instance | medium | attempted |
...
| Derive a claim only from a verified, exact-subject, compatible evidence atom | Makes every grant traceable to one typed support item | `claim_derivation.py`, 16/16 matrix decisions, three unsupported social claims denied | Parser-to-model refinement | low | attempted |
