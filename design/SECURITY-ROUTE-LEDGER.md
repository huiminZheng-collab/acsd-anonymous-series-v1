...
| Generate an independent key per unrelated lineage | Removes the direct equality edge while preserving normal exact-release verification | The third experimental release exposes a distinct key ID; current keygen already supports the workflow | Recovery/privacy study and a pre-release policy lint | low | attempted |
