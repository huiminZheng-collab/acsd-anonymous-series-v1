...
| ID | 场景 | 期望 |
|---|---|---|
| M-APP-01 | 正确私钥 approve | exit 0；endorsement 出现在 endorsements/ |
...
| M-APP-06 | approve 后篡改 release.json 再 approve | 新签名绑定篡改后字节（允许），但最终 verify 失败 —— 验证"签名绑字节"语义 |
...
| M-FIN-03 | TSA 不可达，无 --allow-untimestamped | exit 4；状态回退 awaiting-approvals；已有 endorsement 保留 |
...
| M-FIN-08 | 事后补时间戳（finalized-untimestamped → finalized） | 允许；新 tsq/tsr 入包 + manifest 更新 |
...
| M-VER-07 | 合法 finalized-untimestamped | `DEGRADED`（非 VALID）；non-claims 不含 EXTERNALLY_NOT_AFTER |
| M-VER-08 | 带 TSA 回执的目录：离线验证 tsq/tsr/chain/genTime | 输出 `EXTERNALLY_NOT_AFTER`；不输出创作时间类声明 |
| M-VER-09 | TSA 回执被换（另一个 tsr） | exit 1 `RECEIPT_SUBJECT_MISMATCH` |
| M-VER-10 | awaiting-approvals 中间态目录 | `INCOMPLETE`；列出缺的 key；exit 1 |
...
| M-REL-02 | 多作者 team 用 `acsd release` | 报错并提示分步流程；exit 2 |
...
- 现有 v2 unittest（14 pass + 3 skip）与 demo 验证保持不变。
