# CLI 测试矩阵（accompanying CLI-SPEC.md v3.0）
...
| M-INIT-06 | `--line` 非法或 `--lineage-threshold` 超出作者数 | exit 2 |
...
- 当前 v3.1.0-rc1 unittest（95 pass + 3 个环境相关 skip）、1,000 个生成式
...
| ID | 场景 | 期望 |
|---|---|---|
| M-LIN-01 | authority 不变的严格 n+1 | 新版作者 approvals 同时满足前序 threshold；`AUTHORIZED_CONTINUATION` |
| M-LIN-02 | 攻击者用新密钥抢占 n+1 | 对象自签可成立，但 `VALID_OBJECT_BUT_UNAUTHORIZED_SUCCESSOR`；不能 finalize |
| M-LIN-03 | 合法更换作者/密钥 | exact transition 同时具有前序 quorum 与新作者 approvals；`AUTHORIZED_TRANSITION` |
| M-LIN-04 | 新版本自行降低旧 threshold | 拒绝；旧 threshold 仍控制当前继承边 |
| M-LIN-05 | 前序预先选择 1-of-2 | 任一已列前序密钥可授权轮换 |
| M-LIN-06 | transition 签名移植到另一候选稿 | payload digest 不匹配，拒绝重放 |
