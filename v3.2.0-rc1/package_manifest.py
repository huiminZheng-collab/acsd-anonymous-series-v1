...
def validate_relative_name(name: str) -> str:
    ...
    if unicodedata.normalize("NFC", name) != name:
        raise ValueError("MANIFEST_PATH_INVALID")
    if any(ord(char) < 32 or ord(char) == 127 or char in _WINDOWS_FORBIDDEN for char in name):
        raise ValueError("MANIFEST_PATH_INVALID")
    ...
    if any(part in {"", ".", ".."} for part in parts):
        raise ValueError("MANIFEST_PATH_INVALID")
    if any(part.endswith((".", " ")) or _WINDOWS_DEVICE.fullmatch(part) for part in parts):
        raise ValueError("MANIFEST_PATH_INVALID")
...
def iter_payload_files(root: pathlib.Path) -> Iterable[Tuple[str, pathlib.Path]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if folded in casefolded:
            raise ValueError("MANIFEST_PATH_COLLISION")
...
def write_manifest(root: pathlib.Path) -> pathlib.Path:
    """Write the root manifest with LF bytes on every supported platform."""
    root = pathlib.Path(root)
    manifest_path = root / MANIFEST_NAME
    with manifest_path.open("w", encoding="ascii", newline="\n") as stream:
        stream.write(build_manifest_text(root))
