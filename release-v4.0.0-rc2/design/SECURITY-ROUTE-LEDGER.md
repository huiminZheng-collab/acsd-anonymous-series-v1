...
| Precommit recovery authority | Avoid permanent freeze after key loss without post-hoc impersonation | TUF-style threshold/rotation patterns show the design family; 6 recovery-focused Lean theorem declarations, `test_recovery_authorization.py`, and the public-CLI adversarial runner exercise exact-transition binding and rejection cases | Privacy/linkability, custody, and real-team recovery-drill review | high | attempted |
...
## 2026-09-14 recovery and revocation decisions

| Approach | Target or obstruction | Evidence | Missing check | Cost | Status |
|---|---|---|---|---|---|
| Treat a later `revoked: true` assertion as erasing an earlier valid signature or transition | Immutable signature evidence remains mathematically valid; an offline verifier cannot infer a globally latest notice from one partial view | Existing same-slot fork semantics; RFC 9580 distinguishes compromise from retirement; RFC 9162 and Key Transparency require consistency monitoring or gossip for view completeness | None for retroactive erasure; it contradicts the evidence model | low | ruled out |
| Let the unavailable online authority nominate recovery keys after loss | Reopens fresh-key lineage capture because the alleged recovery authority was not bound by the predecessor | Existing n+1 attacker fixture is structurally identical at this boundary | None; prior commitment is necessary | low | ruled out |
| Reconstruct an old online signing key from escrow or secret shares | Restores the same authority but removes an auditable distinction between ordinary and recovery use and concentrates old-key compromise risk | Secret sharing changes key custody, not the verifier's authorization predicate | Operational HSM/threshold-signing study | high | unexplored |
| Precommit a disjoint recovery key set and threshold, then require its quorum over one exact authority-changing transition plus normal child approval | Restores conditional liveness without granting unspecified future bytes or inventing a post-hoc authority | TUF offline-threshold rotation family; RFC 9580 escrowed specific revocation-signature principle; exact-object plan, adversarial public-CLI runner, accepted/rejected Python fixtures, and six recovery-focused Lean declarations with audit | Privacy/linkability, custody, and real-team recovery-drill review | medium | attempted |
| Give a recovery-authorized child silent global precedence over every ordinary same-slot child | A verifier may not have received the recovery edge or the competing edge; local acceptance cannot prove view completeness | RFC 9162 and the IETF Key Transparency architecture separate inclusion/consistency from gossip or third-party monitoring | Transparency/pinning deployment profile | high | unexplored |

## 2026-09-14 local private-key protection decisions

| Approach | Target or obstruction | Evidence | Missing check | Cost | Status |
|---|---|---|---|---|---|
| Keep generated PKCS#8 PEM keys unencrypted by default without a protected alternative | Key-file theft immediately enables signatures; filesystem permissions alone do not protect copied files | Existing CLI emitted `NoEncryption()` and D-09 identified the product boundary | None; a lower-friction protected profile is needed | low | ruled out as the sole author profile |
| Add an optional interactive encrypted PKCS#8 profile | Protect a local key file at rest while preserving the existing public-key/signature wire format | `key_material.py`, `keygen --encrypt`, successful release, wrong-passphrase, confirmation, and noninteractive-failure tests | Cross-platform terminal and backup usability | low | attempted |
| Accept a passphrase through argv, environment, JSON, or a release artifact | Such values can leak through shell history, process inspection, logs, CI, manifest closure, or public packaging | CLI grammar deliberately has no such option; regression tests inspect only interaction and failure behavior | None; the interface violates the secret boundary | low | ruled out |
| Standardize a portable OS keystore, FIDO2/HSM, or remote signer now | Stronger custody may improve the service level but adds platform-specific APIs, export/recovery policy, and a substantially larger trusted surface | No adapter or device matrix is presently validated | Cross-platform abstraction and hardware/backup tests | high | unexplored |
