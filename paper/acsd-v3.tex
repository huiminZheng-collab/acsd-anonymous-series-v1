...
\begin{enumerate}
  ...
  \item a signature-complete approval-set timestamp subject and a small typed
        scoped-statement core for atomic event disclosure and per-slot identity
        unblinding; and
...
Identity unblinding is an external per-slot sidecar binding the exact
...
statement that the TSA saw that complete target-and-signature set no later than its recorded
...
disclosure signatures rather than self-reported signer identifiers.
...
Python unit tests & 79 passed; 2 environment-dependent tests skipped locally \\
...
PEC, lineage, and scoped-claim theorems & 34; no \texttt{sorry}/\texttt{admit} \\
\bottomrule
\end{tabular}
\end{table}
...
events, a closed outcome type, typed target-versus-approval-set time evidence,
typed scoped statements, and lineage authorities and transitions. Its 34 theorems
...
These 34 are PEC, lineage, scoped-statement, and cross-layer composition theorems, not a recount of the
...
The profile builds on established foundations. PROV-DM supplies a general vocabulary for entities, activities, agents, derivations, and bundles~\cite{prov}. Software Heritage identifiers provide intrinsic identifiers for software objects~\cite{swhid}; Trusty URIs provide hash-bearing immutable artifact identifiers~\cite{trusty}. COSE defines the signed binary envelope used for endorsements~\cite{cose}. SCITT specifies a broader architecture for signed statements and transparency receipts~\cite{scitt}. RFC~3161 specifies the timestamp protocol~\cite{rfc3161}; RFC~9921 explicitly distinguishes COSE-then-timestamp from timestamp-then-COSE~\cite{rfc9921}. ACSD's approval-set closure follows the former ordering principle without embedding the timestamp token in a COSE header. TUF uses predecessor and successor root thresholds to preserve trust through root-key updates~\cite{tufspec}; in-toto binds supply-chain steps to authorized functionaries~\cite{intoto}. These are the closest authorization patterns, although neither specifies anonymous scholarly WorkID succession.
