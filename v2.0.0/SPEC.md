...
Status: v0.1 reference implementation in progress. The terms **MUST** and
**MUST NOT** describe the profile contract, not a deployed standard. The
Python reference core and demo implement the binding rules; Lean compilation
and production cryptographic packaging remain separate acceptance gates.

## 1. Purpose and narrow security goal

A Provenance Evidence Capsule (PEC) binds an exact scholarly release to:

1. an existing, signed anonymous authorship-governance statement;
2. a sequence of commitments to research materials such as a Git snapshot,
   a research note, or a key human--AI dialogue; and
3. optional, separately verified external receipts over the exact PEC digest.
...
- `AuthorshipGovernanceStatement` is the existing signed object for byline
  ...
  independently verified under the coauthor-governance profile.
...
The issuer signs the canonical body using the existing ACSD envelope. Every
key named by `required_pec_approval_key_ids` MUST separately sign that exact
body. For v0.1 this set MUST equal the complete `PaperRelease.authors` key
set; a subset policy is deliberately not supported. A PEC is accepted only if
all required signatures, every referenced object, every event chain link, and
every policy condition verify.
...
It is signed and included in the PEC body so that neither an issuer nor a
service can later change the meaning of existing evidence.
...
| `EXTERNALLY_NOT_AFTER` | policy-approved RFC 3161-style receipt over the exact `pec_digest` | first creation; global priority; originality |
| `WITNESS_OBSERVED` | policy-approved quorum of distinct witness/log receipts | trusted wall-clock time; global visibility |
| `EQUIVOCATION` | two valid, incompatible statements for one policy-defined exclusive slot | motive; human attribution |
| `INDETERMINATE` | missing receipt, missing allowed disclosure, unavailable service, or insufficient quorum | any negative social conclusion |
...
```json
{
  ...
  "required_capabilities": {
    ...
    "WITNESS_OBSERVED": ["policy-approved-witness-quorum"]
  },
...
An external sidecar is not included in the immutable PEC body.  It is a
separate object that names `pec_digest`, has its own canonical bytes, and is
verified under a policy-pinned trust anchor.

- **RFC 3161 sidecar:** retains the raw nonce-bearing request and response,
  verifies the TSA chain and exact message imprint over `pec_digest`, and
...
No receipt may be replayed from an old PEC to a revised PEC, because the
imprint and sidecar subject digest must equal the exact current `pec_digest`.

## 9. Verification procedure

1. Parse canonical PEC bytes, re-canonicalize, and reject any byte mismatch.
2. Verify every PEC approval signature and each referenced ACSD
...
3. Recompute every event digest and enforce the event sequence and predecessor
   chain.
4. Recompute `pec_digest`; verify every sidecar and disclosure binds that
   digest exactly.
...
`TIME_CAPABILITY_MISSING`, `CLAIM_NOT_AUTHORIZED`, and
`SOCIAL_CLAIM_FORBIDDEN`.

## 10. Formalization boundary

The Lean core should model authenticated objects, exact digest bindings,
policy-authorized capability derivation, event-chain order, and disclosure
membership.  Hash collision resistance, signature unforgeability, UTF-8/JSON
parsing, encryption secrecy, RFC 3161 operation, and transparency service
governance remain assumptions attached to the corresponding components.
