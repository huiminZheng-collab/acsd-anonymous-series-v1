...
& $py design/canonical_diff_runner.py
if ($LASTEXITCODE -ne 0) { throw "Canonical differential tests failed with exit code $LASTEXITCODE" }
& $py design/canonical_fuzz_runner.py
if ($LASTEXITCODE -ne 0) { throw "Canonical fuzz tests failed with exit code $LASTEXITCODE" }
& $py design/benchmark_core.py
if ($LASTEXITCODE -ne 0) { throw "Core benchmark failed with exit code $LASTEXITCODE" }
& $py verify_v1_fixture.py
if ($LASTEXITCODE -ne 0) { throw "v1 fixture verification failed with exit code $LASTEXITCODE" }
...
if ($lakePath) {
  ...
  try {
    ...
    & $lakePath env lean AxiomAudit.lean
