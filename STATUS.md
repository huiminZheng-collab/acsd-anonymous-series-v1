...
- Python: 109 tests pass locally; the live TSA test and two unavailable Windows
...
- Lean 4.33.1: build succeeds; 59 PEC/lineage/scoped-appraisal/transcript
  theorems; no
...
projections, signer-set uniqueness, event sequence, window bounds, and one
exact Merkle fact before constructing appraisal atoms. Its composed soundness
...
- connect exact JSON transcript bytes to the new Lean transcript structure,
  then extend the approval/event profile to identity, approval-set time, and
  lineage.
