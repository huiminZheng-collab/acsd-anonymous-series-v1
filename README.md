# ACSD v3.1.0-rc1 Authorized Scholarly Lineage and Provenance Evidence Capsule
...
- 141 Python tests passing, with the live-network and two unavailable Windows
...
- `canonical_json.py`, `bundle_validation.py`, `release_adapter.py`, and
  `claim_derivation.py`: the restricted byte format, single pure PEC/policy
  validator, release projection, and typed granting kernel respectively;
...
- `release-v3.1.0-rc1/`: self-contained deterministic release candidate;
  `release-v3.0.0/` and the earlier
...
python verify_release.py release-v3.1.0-rc1
python release-v3.1.0-rc1/check.py --artifact
python build_release.py --check release-v3.1.0-rc1
```

The installable wheel and the immutable paper/evidence snapshot are separate
artifacts. The root tree and candidate identify as `3.1.0rc1`;
`release-v3.0.0/` remains a frozen historical package and is not rebuilt from
later source.
