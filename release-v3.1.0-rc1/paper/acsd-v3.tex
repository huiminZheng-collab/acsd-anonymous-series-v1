...
\begin{enumerate}
  ...
  \item an executable prototype with one-command release, revision, and
        ...
        checks, generated differential testing, and a Lean model of the
        composition rules.
\end{enumerate}
...
Python unit tests & 95 passed; 3 environment-dependent tests skipped locally \\
...
PEC, lineage, and scoped-claim theorems & 39; no \texttt{sorry}/\texttt{admit} \\
\bottomrule
\end{tabular}
\end{table}
...
The v3 Lean development uses typed abstract records representing approval
targets, approvals, digest-linked events, a closed outcome type, typed
target-versus-approval-set time evidence, typed scoped statements, and lineage
authorities and transitions. Its 39 theorems
...
The scoped layer additionally proves that a grant requires signature, exact
scope, signer authorization and policy membership; identity evidence cannot
grant an event-opening claim; a missing slot prevents full-byline status; and
a target-only timestamp cannot establish approval-set time.
The evidence layer represents an accepted evidence set extensionally and proves
that every claim derived from a union has a compatible, verified, exact-subject
supporting atom in at least one component. Unsupported social claims have no
compatible evidence constructor. This is an abstract composition invariant,
not a refinement proof from accepted parser bytes to Lean values.
...
These 39 are PEC, lineage, scoped-statement, and cross-layer composition theorems, not a recount of the
...
unblinding, and authorized succession.
