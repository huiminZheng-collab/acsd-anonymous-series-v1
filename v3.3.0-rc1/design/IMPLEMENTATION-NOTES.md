...
- `artifact_io.py`：规范 JSON 文件与路径归属边界；
- `key_material.py`：私钥/公钥/证书的加载和 key-id 文件绑定；
- `lineage_adapter.py`：父子边文件结构与前序阈值签名适配；
- `release_verifier.py`：与 argparse/作者命令无关的完整离线验证编排。

`acsd.check_bindings` 与 `pec_core.validate_pec` 保留为兼容入口，但不再各自
实现安全规则。相对 1478 行的整理起点，`acsd.py` 已减少约 682 行至约
796 行；`pec_core.py` 减少约 112 行至约 105 行。
新增模块的目的不是追求仓库总行数下降，而是让每条安全规则只有一个权威
实现。审批集合的文件收集和纯集合验证也已经分开。对象、I/O 适配和完整
验证器现已从 CLI 中移出；`acsd.py` 只保留命令路由与作者侧工作流。没有
新的复用或测试边界前，不再为了目录形式继续拆包。
...
| canonical/key/lineage/release verification | `artifact_io.py` / `key_material.py` / `lineage_adapter.py` / `release_verifier.py` | 单向进入 protocol/claim kernel，均不依赖 CLI；旧名称由 `acsd.py` 原对象导出 |
