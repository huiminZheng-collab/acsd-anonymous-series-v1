...
class TestEventDisclosure(unittest.TestCase):
    def setUp(self):
        ...
        self.pec = {
            ...
            "claim_policy": {
                "permitted_outcomes": ["COMMITTED_EVIDENCE_MATCH"]
            },
        }
        self.disclosure = {
    ...
    def test_atomic_valid_disclosure(self):
        result = verify_event_disclosure(
            ...
            accepted_pec_digest=digest(self.pec),
        )
    ...
    def test_policy_and_cross_pec_replay_are_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "DISCLOSURE_POLICY_INVALID"):
            verify_event_disclosure(
                ...
                accepted_pec_digest=digest(changed_policy),
            )
        ...
        with self.assertRaisesRegex(ValueError, "DISCLOSURE_BINDING_MISMATCH"):
            verify_event_disclosure(
                ...
                accepted_pec_digest=digest(other),
            )
    ...
    def test_claim_policy_and_accepted_pec_are_required(self):
        forbidden = copy.deepcopy(self.pec)
        forbidden["claim_policy"]["permitted_outcomes"] = []
        body = copy.deepcopy(self.disclosure)
        body["pec_digest"] = digest(forbidden)
        with self.assertRaisesRegex(ValueError, "CLAIM_NOT_AUTHORIZED"):
            verify_event_disclosure(
                body, forbidden, self.public_by_id, self.signatures(body),
                accepted_pec_digest=digest(forbidden),
            )
        with self.assertRaisesRegex(ValueError, "PEC_NOT_ACCEPTED"):
            verify_event_disclosure(
                self.disclosure, self.pec, self.public_by_id, self.signatures(),
                accepted_pec_digest="0" * 64,
            )

    def test_unknown_fields_and_malformed_opening_are_rejected(self):
        extra = copy.deepcopy(self.disclosure)
        extra["self_reported_approval"] = True
        with self.assertRaisesRegex(ValueError, "DISCLOSURE_FIELDS"):
            verify_event_disclosure(
                extra, self.pec, self.public_by_id, self.signatures(extra),
                accepted_pec_digest=digest(self.pec),
            )
        malformed = copy.deepcopy(self.disclosure)
        malformed["opened_material"][0]["salt"] = "00"
        with self.assertRaisesRegex(ValueError, "DISCLOSURE_OPENING_INVALID"):
            verify_event_disclosure(
                malformed, self.pec, self.public_by_id, self.signatures(malformed),
                accepted_pec_digest=digest(self.pec),
            )
