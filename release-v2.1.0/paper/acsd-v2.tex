...
\begin{enumerate}
  ...
  \item an acyclic unanimous-approval target and closed capability algebra that prevent unsigned policy amplification and approval reuse after governance or PEC substitution; and
...
The inherited v1 series fixtures contain per-author release endorsements.  The
v2 command-line profile additionally constructs an
\texttt{acsd-approval-target/v1} object containing the work id, sorted required
key set, and the SHA-256 digests of the release, governance statement, and PEC.
Each required key issues a tagged COSE Sign1 signature over the same canonical
target.  Verification recomputes every digest and also recomputes each key id
from the supplied public key. Thus replacing a governance statement and PEC,
even coherently, invalidates the target; recomputing the target cannot reuse the
old signatures. A verifier can state that the pseudonymous keys jointly
approved this exact tuple, but cannot infer who controls them without separate
enrollment evidence.
...
Real RFC 3161 interoperability & 1 freeTSA response verified \\
...
rejected at their intended boundary.

The measurements remain prototype-scale rather than a throughput claim. In one
local Windows run, five repetitions of an in-memory workload that canonicalizes
...
only checks that the reference core has no immediate superlinear surprise.

The Lean development uses concrete approval targets, approvals, digest-linked
